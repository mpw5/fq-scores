...
var twss = require('twss');
  ...
  if (channel === "friday-question"){
      ...
      // process an emoji
          ...
          let message = Object.assign({ "response_type": "in_channel", text: userAwardedPoints + " has been awarded a " + pointsAwarded + " by @" + payload.user_name + " " + comment});
        ...
        .then(function(){
          ...
          // get the user's current score from the database
           datastore.get(userAwardedPoints) // get the count for the user_id
              .then(function(score){
                let message = Object.assign({ "response_type": "in_channel", text: userAwardedPoints + " has been awarded " + Number(pointsAwarded).toLocaleString() + " points by @" + payload.user_name+ " " + comment});
      ...
      // anything else is invalid
          ...
          let message = Object.assign({ text: "Sorry. That's not a valid instruction. Try a little harder next time. (That's what she said.)"});
        ...
        let message = Object.assign({text: "This command only works in the #friday-question channel. If you would like to know more, come and talk to us. We're a friendly bunch."});
...
function getResults(result, user_name) {
  ...
  return Object.assign({ "response_type": "in_channel", text: resultText });
...
function getConnected() {
  ...
  let rtm = new RtmClient(SLACK_API_TOKEN, {
    logLevel: 'error', 
